(function(){
  var DEBUGGING, STANDALONE, map, LANG, MOEID, HASHOF, XREFLABELOF, TITLEOF, STARRED, res$, key, LRU, isCordova, isQuery, isDroidGap, isDeviceReady, isMobile, isApp, isWebKit, isGecko, isChrome, widthIsXs, entryHistory, INDEX, XREF, CACHED, GET, e, playing, player, seq, getEl, callLater, MOE, trs_lookup, CJKRADICALS, SIMPTRAD, ref$, httpMap, Consonants, Vowels, Tones, re, C, V, LoadedScripts, split$ = ''.split, replace$ = ''.replace, join$ = [].join, slice$ = [].slice;
  DEBUGGING = false;
  STANDALONE = false;
