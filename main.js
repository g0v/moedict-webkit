...
(function(){
  ...
  function initAutocomplete(){
    ...
    return $('#query').autocomplete({
      ...
      source: function(arg$, cb){
        ...
        if (!(LANG !== 't' || /[^\u3100-\u312D]/.exec(term) || /[,;0-9]/.exec(term))) {
          return zhuyin_lookup(term, cb);
        }
  ...
  LookupURL = isCordova || /^https:\/\/(?:www.)?moedict.tw/i.exec(document.URL) ? "/lookup" : 'http://su-lip.magistry.fr/lookup';
  trs_lookup = function(term, cb){
    return GET(LookupURL + "/trs/" + term, function(data){
      return cb(split$.call(data, '|'));
    });
  };
  zhuyin_lookup = function(term, cb){
    return GET(LookupURL + "/zhuyin/" + term, function(data){
      return cb(split$.call(data, '|'));
    });
  };
  alter_input = function(ev){
    var input_char, char, offset, txt, newtxt;
    if (!(LANG === 't')) {
      return;
    }
    ...
    input_char == null && (input_char = ev.keyCode);
    ...
    return $('#query')[0].selectionEnd = offset + 1;
  };
  convert_zhuyin = function(chr){
    switch (chr) {
    case 126:
      return "～";
    case 33:
