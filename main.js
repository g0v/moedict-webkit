...
(function(){
  ...
  window.playAudio = function(el, url){
    ...
    play = function(){
      ...
      urls = [url];
      if (/ogg$/.exec(url) && canPlayMp3() && !/\bGecko\/\b/.test(navigator.userAgent)) {
  ...
  window.doLoad = function(){
    var fontSize, saveFontSize, cacheLoading, pressAbout, pressErase, pressBack, init, grokVal, grokHash, fillQuery, prevId, prevVal, bucketOf, lookup, doLookup, htmlCache, fetch, loadJson, setPinyinBindings, setHtml, loadCacheHtml, fillJson, keyMap, fillBucket, lang, i$, ref$, len$, results$ = [];
    if (!isDeviceReady) {
      return;
    }
    if (isCordova) {
    ...
    function fn2$(lang){
      return GET(lang + "/=.json", function(it){
        return $(".taxonomy." + lang).parent().replaceWith(renderTaxonomy(lang, $.parseJSON(it)).children());
      }, 'text');
    }
  };
