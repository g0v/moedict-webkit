...
(function(){
  var DEBUGGING, STANDALONE, LANG, MOEID, HASHOF, XREFLABELOF, isCordova, isDroidGap, isDeviceReady, isMobile, ref$, isApp, isWebKit, widthIsXs, entryHistory, INDEX, XREF, CACHED, GET, e, playing, player, seq, getEl, callLater, MOE, CJKRADICALS, SIMPTRAD, httpMap, Consonants, Vowels, Tones, re, C, V, LoadedScripts, split$ = ''.split, replace$ = ''.replace, join$ = [].join, slice$ = [].slice;
  DEBUGGING = false;
  STANDALONE = false;
  ...
  if (isCordova || ((ref$ = window.locationbar) != null ? ref$.visible : void 8) === false) {
    isApp = true;
  }
  ...
  function render(json){
    ...
    result = ls(heteronyms, function(arg$){
      var id, audio_id, ref$, bopomofo, pinyin, trs, definitions, antonyms, synonyms, variants, cnSpecific, basename, mp3;
      ...
      cnSpecific = '';
      if (/^<br>/.exec(bopomofo)) {
        bopomofo = replace$.call(bopomofo, /^<br>/, '');
        pinyin = replace$.call(pinyin, /^<br>/, '');
        cnSpecific = 'cn';
      }
      if (!/</.test(title)) {
