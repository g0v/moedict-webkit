  ...
  window.doLoad = function(){
    ...
    init = function(){
      ...
      $('body').on('click', '.results .star', function(){
        var key;
        key = HASHOF[LANG] + "" + prevId + "\n";
        if ($(this).hasClass('icon-star-empty')) {
          STARRED += key;
        } else {
          STARRED = replace$.call(STARRED, key + "", '');
        }
        $(this).toggleClass('icon-star-empty').toggleClass('icon-star');
        return setPref('starred', STARRED);
      });
    ...
    setHtml = function(html){
      return callLater(function(){
        ...
        html = html.replace('<!-- STAR -->', ~STARRED.indexOf(HASHOF[LANG] + (prevId + "\n")) ? "<span class='star iconic-color icon-star' title='加入字詞記錄簿'></span>" : "<span class='star iconic-color icon-star-empty' title='已加入記錄簿'></span>");
        $('#result').html(html);
