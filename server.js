...
>>>>>>> master
(function(){
  var fs, LTMRegexes, indexBody, React, ref$, Result, decodeLangPart, XREF, i$, len$, trim, defOf, HASHOF, wt2font, font2name, fontOf, iconv, fixMojibake, replace$ = ''.replace, split$ = ''.split;
  fs = require('fs');
  LTMRegexes = {};
  ...
  XREF = {};
  for (i$ = 0, len$ = ['a', 't', 'h', 'c'].length; i$ < len$; ++i$) {
    (fn$.call(this, ['a', 't', 'h', 'c'][i$]));
  }
  ...
  fontOf = function(it){
    ...
    if (/srcx/i.exec(it)) {
      return 'SourceHanSansTWExtraLight';
    }
    if (/srcl/i.exec(it)) {
      return 'SourceHanSansTWLight';
    }
    if (/srcn/i.exec(it)) {
      return 'SourceHanSansTWNormal';
    }
    if (/srcr/i.exec(it)) {
      return 'SourceHanSansTWRegular';
    }
    if (/srcm/i.exec(it)) {
      return 'SourceHanSansTWMedium';
    }
    if (/srcb/i.exec(it)) {
      return 'SourceHanSansTWBold';
    }
    if (/srch/i.exec(it)) {
          ...
          return html(attrs, function(){
            return head(function(){
              ...
              if (!this.segments) {
                ...
                props = {};
                if (/^\[\s*\[/.exec((this.json || '').toString())) {
                  props = {
                    id: id,
                    type: 'radical',
                    terms: (this.json || '').toString(),
                    H: h
                  };
                } else if (/^\[/.exec((this.json || '').toString())) {
                  props = {
                    id: id,
                    type: 'list',
                    terms: this.json || '',
                  ...
                  props = JSON.parse(this.decodeLangPart(h, (this.json || '').toString()));
                  fillProps();
                }
